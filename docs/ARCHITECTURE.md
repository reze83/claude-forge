...
| permission-request.sh | PermissionRequest  | _(kein matcher)_              | Symlink + Plugin | Permission-Anfragen loggen + opt-in Gate (CLAUDE_FORGE_PERMISSION_GATE=1)                                                                                    |
...
| `modify_input(json)`  | PreToolUse updatedInput: outputs allow decision with modified tool input JSON. Used by input-sanitizer.sh.                                                            |
...
| `notify(message)`     | Desktop notification (async): WSL→PowerShell toast, Linux→notify-send. Runs in background subshell.                                                                   |
| `log_event(file,msg)` | Timestamp + message append to log file with automatic directory creation.                                                                                             |
...
| Hook Metrics (trap)   | EXIT trap logs ms-precision execution time per hook to `hooks-debug.log` (only when `CLAUDE_FORGE_DEBUG=1`). Uses `date +%s%N` with macOS `gdate`/`SECONDS` fallback. |
...
PreToolUse Hooks koennen Inputs modifizieren (via `modify_input()` from lib.sh):
...
| ---------------- | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| test-hooks.sh    | 252   | bash-firewall (63: basic+bypass+subshell/pipe/backtick/herestring+force-with-lease+editors+mkfs/dd+dry-run+local-patterns), protect-files (29: basic+case-insensitive+allowlist+tampering+non-ASCII+dry-run), secret-scan (16: pre+post+pragma+dry-run), auto-format (2), url-allowlist (22: public+private+deny-json+empty+IPv6), pre-write-backup (7: opt-in+skip-tmp+skip-node_modules+bak-creation), session-logger (3: basic+log-rotation), session-start (2), setup (3: basic+additionalContext+forgeVersion), post-failure (2), pre-compact (2), task-gate (2), teammate-gate (4: opt-in+non-git+dirty-tree+clean-tree), subagent-start (2), subagent-stop (2), stop (3: basic+skip-loop+async-notify), smithery-context (8: basic+mock+mcp-true+mcp-false+cache-write+cache-hit+cache-expired), hook-metrics (2: debug-log+ms-precision), input-sanitizer (14: passthrough+ANSI+path+CRLF+protocol+error), config-change (9: basic+log+lock+policy-exempt+stderr+error), negative/error (22: corrupt JSON+empty stdin+missing fields+oversized) |
| test-update.sh   | 6     | --help, VERSION, Nicht-Git-Repo, --check                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| test-install.sh  | 35    | Install/Uninstall Lifecycle (Hardlinks, rekursive Skills, VERSION deploy, Repo-Marker), Symlink-Migration, QA-Tools Section, Settings-Merge Edge Cases (corrupt JSON, empty settings, dry-run), Array-Merge (template+user union for permissions/domains), MCP-Sync (dry-run)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| test-codex.sh    | 30    | Codex Wrapper (error handling, timeout, model flag, render_template, context-file, template, is_transient_error, --no-retry, live)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| test-plugin.sh   | 9     | Plugin-Mode (JSON validity, name, description, no hooks field, version sync, directories, CLAUDE_PLUGIN_ROOT)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
Total: 336 tests (252 hooks + 35 install + 9 plugin + 6 update + 33 codex + 1 validate).

## Architecture Decision Records (ADRs)

| ADR  | Title                        | Status   | Summary                                                                                  |
| ---- | ---------------------------- | -------- | ---------------------------------------------------------------------------------------- |
| 0001 | LLM-Based Hook Handlers      | Accepted | `type: prompt`/`agent` hooks not deployed by default (token cost); opt-in templates only |
| 0002 | ConfigChange Hook Protection | Accepted | Audit log default, opt-in blocking via `CLAUDE_FORGE_CONFIG_LOCK=1`                      |

ADR files are in `docs/adr/`.
